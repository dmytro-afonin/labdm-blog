...
| `vercel pull`                        | `bunx vercel pull --yes --environment=preview --token="$VERCEL_TOKEN"` — downloads project settings for **preview** into `.vercel/`.                                           |
...
| Deploy                               | `bunx vercel deploy --yes --token="$VERCEL_TOKEN"` — **remote** build (so Sensitive Neon URLs work for `db:migrate` in `buildCommand`); stdout must be an `https://` URL.      |
...
| Deploy                               | `bunx vercel deploy --prod --yes --token="$VERCEL_TOKEN"` — remote build + migrate via `buildCommand` (success URL for GitHub is `siteConfig.url`, not deploy stdout).                                                                                |
